...
import {
  ...
  Wallet, Info, PanelRightClose, PanelRightOpen,
} from 'lucide-react'
import {
...
const BOSLUK: Filtreler = {
  q: '', sektor: '', asama: '', temsilci: '', oncelik: '', vade: '', bugun: false,
}
...
function activeFilterCount(f: Filtreler) {
  return [f.sektor, f.asama, f.temsilci, f.oncelik, f.vade].filter(Boolean).length
    + (f.bugun ? 1 : 0)
}
...
export function MusterilerClient({
  ...
  useEffect(() => {
    ...
    fetchRecords(f)
    fetchStats(f)
  }, [debouncedQ, filtreler.sektor, filtreler.asama, filtreler.temsilci, filtreler.oncelik, filtreler.vade, filtreler.bugun]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-[15px]">
      ...
      {/* ── Hero ──────────────────────────────────────────────────────────── */}
      <section
        className="relative h-[150px] rounded-[16px] overflow-hidden text-white flex items-center justify-between px-[31px] shadow-sm"
      ...
      >
        ...
        {/* Stat tiles */}
        <div className="relative flex items-center gap-[13px] pr-[10px]">
          {([
            { Icon: Flame,          n: sicakKpi,               label: 'Sıcak'  },
            { Icon: Clock3,         n: counts.bugunAranacak,   label: 'Bugün'  },
            { Icon: MessageSquare,  n: counts.yanitBekleyen,   label: 'Yanıt'  },
          ] as const).map(({ Icon, n, label }) => (
            <div key={label}
              className="h-[70px] w-[86px] rounded-[14px] border border-white/10 bg-white/5 grid place-items-center">
              <div className="text-center">
                <Icon size={17} className="mx-auto mb-[5px] text-pink-300" />
                <div className="text-[24px] font-black leading-none">{n}</div>
                <div className="mt-[3px] text-[11px] text-white/80">{label}</div>
              </div>
            </div>
          ))}
        </div>
