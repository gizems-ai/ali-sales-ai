...
import { resolveTenant, DEFAULT_TENANT_ID, type TenantConfig } from './tenants'
...
export interface KullanicıProfili {
  userId: string
  rol: Rol
  tenantId: string           // Clerk publicMetadata.tenant → default: 'sigortan_biz'
  temsilciAdi?: string
...
function satir(
  overrides: Partial<Record<Ekran, EkranYetkisi>>,
  base: EkranYetkisi,
): RolSatiri {
...
export const YETKI_MATRISI: Record<Rol, RolSatiri> = {
  ...
  satış_temsilcisi: satir(
    {
      musteriler:   y(true, true,  false),
      satis_sureci: y(true, true,  false),
      teklifler:    y(true, true,  false),
      takvim:       y(true, true,  false),
      gorevler:     y(true, true,  false),
    },
    y(true, false, false),
  ),
  ...
  operasyon_temsilcisi: satir(
    {
      policeler:   y(true, true, false),
      yenilemeler: y(true, true, false),
      komisyonlar: y(true, true, false),
      takvim:      y(true, true, false),
      gorevler:    y(true, true, false),
    },
    y(true, false, false),
...
export function izolasyonBelirle(profil: KullanicıProfili): IzolasyonSiniri {
  ...
  if (profil.rol === 'satış_temsilcisi' && profil.temsilciAdi) {
    return { tip: 'temsilci', ad: profil.temsilciAdi }
  }
...
/** Senkron: mevcut profil nesnesinden tenant config döner. */
export function getTenantConfig(profil: KullanicıProfili | null | undefined): TenantConfig {
  return resolveTenant(profil?.tenantId)
}

/** Asenkron: Clerk'ten profili okuyup tenant config döner. API route'larında kullan. */
export async function getTenantConfigFromRequest(): Promise<TenantConfig> {
  const profil = await getKullanicıProfili()
  return resolveTenant(profil?.tenantId)
}

// ─── Clerk'ten profil oluşturma ──────────────────────────────────────────────
...
export async function getKullanicıProfili(): Promise<KullanicıProfili | null> {
  ...
  const tenantId = typeof meta.tenant === 'string' && meta.tenant
    ? meta.tenant
    : DEFAULT_TENANT_ID

  if (meta.rol === 'admin') {
    return { userId: user.id, rol: 'admin', tenantId }
  }
  if (meta.rol === 'yönetici') {
    return { userId: user.id, rol: 'yönetici', tenantId }
  }
  ...
  // Geriye dönük uyumluluk: eski format { temsilci: "Rüya" } (rol alanı yok)
