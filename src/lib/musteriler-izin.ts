...
export interface MusterilerFiltreler {
  ...
  brans?: 'saglik' | 'elementer' | 'acibadem' | 'crosssell'
}
...
export function buildMusterilerFormula(
  ...
  if (f.brans) {
    switch (f.brans) {
      case 'saglik':
        conds.push(`NOT({Sağlık Vade Tarihi}=BLANK())`)
        break
      case 'elementer':
        conds.push(`OR(NOT({Elementer Vade}=BLANK()),NOT({Elementer Ürün}=BLANK()))`)
        break
      case 'acibadem':
        conds.push(`SEARCH("Acıbadem",{Branş})>0`)
        break
      case 'crosssell':
        conds.push(`AND(NOT({Sağlık Vade Tarihi}=BLANK()),OR(NOT({Elementer Vade}=BLANK()),NOT({Elementer Ürün}=BLANK())))`)
        break
    }
  }
  if (f.q && f.q.trim()) {
